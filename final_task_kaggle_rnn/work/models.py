...
def get_lstm_model_1(vocab_size, embedding_dim, rnn_units):
    inputs = keras.layers.Input(shape=(None,), dtype='int32', name='input_tokens')
    embedding = keras.layers.Embedding(input_dim=vocab_size, output_dim=embedding_dim)(inputs)
    lstm, hidden_state_1, cell_state_1 = keras.layers.LSTM(units=rnn_units, return_sequences=True, return_state=True)(embedding)
    lstm, hidden_state_2, cell_state_2 = keras.layers.LSTM(units=rnn_units, return_sequences=True, return_state=True)(lstm)
    outputs = keras.layers.Dense(units=vocab_size, activation='softmax')(lstm)
...
def get_lstm_model_2(vocab_size, embedding_dim, rnn_units):
    ...
    lstm, hidden_state_2, cell_state_2 = keras.layers.LSTM(units=rnn_units, return_sequences=True, return_state=True)(dropout)
    outputs = keras.layers.Dense(units=vocab_size, activation='softmax')(lstm)
